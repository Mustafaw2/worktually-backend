...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        file_path = os.path.join("lookups", "fixtures", "file-5.json")
            ...
            for city_data in cities_data:
                ...
                if (
                    ...
                    try:
                        ...
                        self.stdout.write(
                            self.style.ERROR(
                                f"State with id '{state_id}' does not exist"
                            )
                        )
                else:
                    self.stdout.write(
                        self.style.ERROR(
                            "Invalid city data: 'id', 'name', 'state_id', 'country_id', 'country_code', 'latitude', and 'longitude' fields are required"
                        )
                    )
