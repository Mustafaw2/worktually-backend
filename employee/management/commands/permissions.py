    ...
    def handle(self, *args, **kwargs):
        ...
        for role_name, permissions in permissions_to_roles.items():
            ...
            for permission_name in permissions:
                permission, perm_created = Permission.objects.get_or_create(name=permission_name)

                # Get or create the PermissionGroup for the permission
                permission_group, group_created = PermissionGroup.objects.get_or_create(name=permission_name)
                permission_group.permissions.add(permission)

                Role_has_Permission.objects.get_or_create(role=role, permission=permission)

                if perm_created:
                    self.stdout.write(self.style.SUCCESS(f'Created permission: {permission_name}'))
            if created:
                self.stdout.write(self.style.SUCCESS(f'Created role: {role_name}'))

        self.stdout.write(self.style.SUCCESS('Permissions successfully added to roles.'))
