...
class JobSearchSerializer(serializers.Serializer):
    ...
    def validate_job_title_id(self, value):
        if value <= 0:
            raise serializers.ValidationError("Job title ID must be a positive integer.")
        return value
    ...
    def search_jobs(self, job_title_id):
        ...
        if not job_posts.exists():
            raise serializers.ValidationError("No job posts found for the given job title ID.")
